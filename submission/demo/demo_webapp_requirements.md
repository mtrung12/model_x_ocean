...
### 5.1 Model artifacts
- The RAG path needs a FAISS dual index. `rag/retriever.py` looks for:
  - dual index: `data/vector_db/essays_dual/vectors.faiss`
  `data/vector_db/essays_dual/` ships in the repo. The embedder uses the public
  `nomic-ai/nomic-embed-text-v1.5` model (downloaded on first use, ~270 MB); no
  local model artifacts are required.
