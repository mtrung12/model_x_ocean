...
Reads one essay and emits a structured profile (~1000 tokens) over the 30
NEO-PI-R facets plus a LIWC-style linguistic fingerprint, used both for
retrieval embedding and as sliced few-shot exemplars at inference time.

The bias-reduction design of these prompts (label-aware counterfactual
framing, anti-genre-bias clause, the "n/e" signal, etc.) is documented in
README.md. Refs: Costa & McCrae 1992, Pennebaker & King 1999,
Mairesse et al. 2007, Schwartz et al. 2013.
"""
...
# Soft ceiling: ~900 tokens expected (30 facets + linguistic block + header).
PROFILER_MAX_TOKENS = 1024

# Signal cell vocabulary. "n/e" = not evidenced (not observable in this genre);
# "none" = observable but neutral.
SIGNAL_VOCAB = ("high", "mod", "low", "none", "n/e")


# (code, name, trait, short_definition); short_definition is a 6-12 word
# psychometric gloss that anchors the model's reading of each facet.
...
# LINGUISTIC fingerprint block: one fixed key per line with a free-text
# value, following Pennebaker/Schwartz's strongest predictors.
