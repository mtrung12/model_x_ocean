...
\emph{linguistic fingerprint}: seven fixed keys, each summarised in a single
free-text line. These keys follow the strongest meta-analytic predictors of
personality in the LIWC and open-vocabulary
literature~\cite{Pennebaker2007TheDA}. They capture genre-level and stylistic
information that the facet schema deliberately suppresses, and they are carried
into the embedded profile text where they contribute to retrieval similarity.
Table~\ref{tab:linguistic-keys} lists the seven keys and what each one records.

\begin{table}[htbp]
  \centering
  \caption{The seven linguistic-fingerprint keys emitted by the profiler,
    with the stylistic property each one summarises.}
  \label{tab:linguistic-keys}
  \small
  \begin{tabular}{ll}
    \toprule
    \textbf{Key} & \textbf{What it records} \\
    \midrule
    \emph{pronouns}  & I/we/you/they density and dominance pattern \\
    \emph{emotion}   & Positive/negative balance and dominant affect \\
    \emph{tense}     & Past/present/future emphasis \\
    \emph{cognitive} & Insight/causal/tentative word density (low/mod/high) \\
    \emph{social}    & Social-process word density (low/mod/high) \\
    \emph{register}  & Informal/mixed/formal and exploratory/focused style \\
    \emph{length}    & Approximate word count, sentence style, lexical diversity \\
    \bottomrule
  \end{tabular}
\end{table}
...
Each evidence cue must be tagged as \texttt{[state]} (temporary or situational)
or \texttt{[trait]} (recurring or cross-situational). The \texttt{<cue\_tally>}
block audits these tags and applies an explicit decision rule: the presumption
is \textit{high} if at least one clear \texttt{[trait]} cue is present \emph{or}
two or more facets are rated \textit{high} in \texttt{<facet\_check>}; otherwise
the presumption is \textit{low}. State cues are treated as weaker evidence than
trait cues but are not discarded entirely --- multiple state cues pointing in the
same direction, or a single extreme cue, are allowed to contribute toward the
high pole.

The \texttt{<verdict>} must be consistent with the \texttt{<cue\_tally>} and
may not conclude \textit{low} solely because supporting cues are state-level
when the facet stage has already rated two or more facets \textit{high}. This
rule couples the final answer to the structured facet evidence rather than to
the model's prior about trait recurrence.

Two trait-specific scoring notes are injected into the user prompt for
Neuroticism and Extraversion, the two traits most susceptible to
surface-vocabulary confounds. For Neuroticism the note instructs the model to
tag situational stressors (exam pressure, fatigue, homesickness) as
\texttt{[state]} and reserve \texttt{[trait]} for explicitly habitual or
chronic emotional patterns. For Extraversion it instructs the model to tag
expressive or enthusiastic writing style as \texttt{[state]} and restrict
\texttt{[trait]} to cues about social energy and orientation. The empirical
effect of these rules on the per-trait bias is examined in
...
enforces a six-block XML output format that tags each evidence cue as
\texttt{[state]} or \texttt{[trait]}, audits them in a structured cue tally,
and requires the verdict to be consistent with the facet-check evidence
before issuing the final label. The experimental evaluation of this pipeline
is presented in Chapter~\ref{chap:experiments}.
